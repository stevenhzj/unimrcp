...
static apt_bool_t xfyun_login()
{
	...
	if (MSP_SUCCESS != ret)
	{
		apt_log(SYNTH_LOG_MARK,APT_PRIO_ERROR,"[xfyun] MSPLogin failed , Error code %d.", ret);
		return FALSE; //登录失败，退出登录
	}
	apt_log(SYNTH_LOG_MARK,APT_PRIO_INFO,"[xfyun] MSPLogin success");
	return TRUE;
}
...
static apt_bool_t xfyun_synth_text_to_speech(const char* src_text, const char* params, mpf_buffer_t *buffer) {
	...
	apt_log(APT_LOG_MARK, APT_PRIO_WARNING,"[xfyun] 合成内容 %s",src_text);
